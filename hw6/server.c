...
void *loginThreadLimit(void* vargs);
...
//-1 if crash, 0 if no crash
int checkClientCrashMainThread(struct user* client, char *cmsg){
	if(cmsg == NULL){
		handleClientCrash(client);
		//pthread_exit(NULL);
		return -1;
...
void verbosePrint(char *message){
	if(verbose){
		...
		colorVerbose();
		sfwrite(&mutex, stdout, message, strlen(message));
...
int findGreatestClientFd()
{
	...
	while(curr!=NULL)
	{
		if(curr->client_socket_fd > biggestFd)
			biggestFd = curr->client_socket_fd;
		curr = curr->next;
	}
	pthread_mutex_unlock(&mutex);
	return biggestFd;
...
void broadcastToUsers(char* msg){
	...
	while(curr != NULL){
		if(curr->connected == 1 && curr->client_socket_fd != -1){
...
void initThreads(){
	if(thread_count <= 0){
		...
		if(sem_init(&queueItems, 0, 0) < 0){
			error("Error initializing semaphore.");
		}
		for(int i=0;i<thread_count;i++){
			//printf("Created %ith thread.\n", i);
			pthread_t some_tid;
			pthread_create(&some_tid, NULL, loginThreadLimit, "hello");
		}
	}
}
...
void *loginThreadLimit(void *vargs){
	while(1){
		struct user* currConnecting = popLoginQueue();
		if(currConnecting != NULL) {
			loginProtocol(currConnecting);
		}
	}
...
int handleClientCommands(struct user* currConnecting){
	...
	if(checkClientCrashMainThread(currConnecting, msg) == -1){
		return 0;
	}
...
void handleOnAccept(struct user* currConnecting){
	if(thread_count <= 0){
...
void selectLoop(){
	...
	while(1){
		...
		while(curr != NULL){
			if(curr->connected == 1){
				FD_SET(curr->client_socket_fd, &readfds);
			}
			curr = curr->next;
		}
		pthread_mutex_unlock(&mutex);

		int biggest = findGreatestClientFd();
		int sret = select(biggest+1, &readfds, NULL, NULL, &timeout);

		for(int i=0;i<= biggest && sret > 0; i++){
			if(FD_ISSET(i, &readfds)){
				if(i == socketfd){
					...
					struct user* currConnecting = createUser(
						...
						-1, //threadID (none currently)
						time(NULL),
						NULL,
						NULL
					);

					handleOnAccept(currConnecting);
				}
				else if(i == 0){
					char last_char;
					...
					struct user* gottenUser = getUserByFD(i);

					if(gottenUser != NULL){
						handleClientCommands(gottenUser);
					}
				}
			}
		}
...
void handleCommand(char *cmd){
	if(cmd[0] != '\0'){
		...
		else if(strcmp(token_cmd[0], "/listq") == 0){
			printLoginQueue();
		}
...
int getCommands(int argc, char** argv){
	...
	if(argc < 3){
		...
		if(strcmp(argv[1], "-h") == 0){
			...
			if(strcmp(argv[2], "-t") == 0){
				...
				else if(argc == 7){ //has accounts file
					thread_count = strtol(argv[3], NULL, 10);
					if(thread_count <= 0){
						printf("Thread count must be greater than 0.\n");
						printHelpMenu();
			...
			else if(argc == 6){ //has accounts file
				thread_count = strtol(argv[2], NULL, 10);
				if(thread_count <= 0){
					printf("Thread count must be greater than 0.\n");
				}
		...
		else{ // ./server port(4444) motd database.db
	...
	if(motd == NULL){
		printf("Must have a message of the day!");
		printHelpMenu();
		return 0;
	}
