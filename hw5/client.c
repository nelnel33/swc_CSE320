...
void checkForChatCommand(char* cmd);
...
struct chat* createChat(char* title, char* name, struct chat* existingChat)
{
	...
	else
	{
		...
		if(pid==0)
		{
			...
			char* newArgv[7] = {"/usr/bin/xterm", "-T", title, "-e", "./chat", fds ,0};
...
void handleHFlag()
{
	printf("Usage: ./client [-hcv] NAME SERVER_IP SERVER_PORT\n");
